...
const int post_buffer_size = 512;
#define NAMELEN 512 
#define VALUELEN 512
#define PREFIX ""
...
stop_effect = 1;
 ...
 void process_response(char schluessel[512], char parameter[512]){

if (strcmp(schluessel, "ledmode") == 0){
  switch_ledmode(strtoul(parameter, NULL, 10));
}if (strcmp(schluessel, "custom_on_h") == 0){
  Serial.println(getValue(parameter, ',', 1).toInt());
  hzr = getValue(parameter, ',', 1).toInt();
  hzb = getValue(parameter, ',', 0).toInt();
  hzg = getValue(parameter, ',', 2).toInt();
  her = getValue(parameter, ',', 1).toInt();
  heb= getValue(parameter, ',', 0).toInt();
  heg = getValue(parameter, ',', 2).toInt();
}if (strcmp(schluessel, "custom_off_h") == 0){
  hzrz = getValue(parameter, ',', 1).toInt();
  hzbz = getValue(parameter, ',', 0).toInt();
  hzgz = getValue(parameter, ',', 2).toInt();
  herz = getValue(parameter, ',', 1).toInt();
  hebz= getValue(parameter, ',', 0).toInt();
  hegz = getValue(parameter, ',', 2).toInt();
  
  
}if (strcmp(schluessel, "custom_on_m") == 0){
  Serial.println(getValue(parameter, ',', 1).toInt());
  mzr = getValue(parameter, ',', 1).toInt();
  mzb = getValue(parameter, ',', 0).toInt();
  mzg = getValue(parameter, ',', 2).toInt();
  mer = getValue(parameter, ',', 1).toInt();
  meb= getValue(parameter, ',', 0).toInt();
  meg = getValue(parameter, ',', 2).toInt();
}if (strcmp(schluessel, "custom_off_m") == 0){
  mzrz = getValue(parameter, ',', 1).toInt();
  mzbz = getValue(parameter, ',', 0).toInt();
  mzgz = getValue(parameter, ',', 2).toInt();
  merz = getValue(parameter, ',', 1).toInt();
  mebz= getValue(parameter, ',', 0).toInt();
  megz = getValue(parameter, ',', 2).toInt();
  
}/*if (strcmp(schluessel, "custom_on_s") == 0){
  Serial.println(getValue(parameter, ',', 1).toInt());
  mzr = getValue(parameter, ',', 1).toInt();
  mzb = getValue(parameter, ',', 0).toInt();
  mzg = getValue(parameter, ',', 2).toInt();
  ...
  meb= getValue(parameter, ',', 0).toInt();
  meg = getValue(parameter, ',', 2).toInt();
}if (strcmp(schluessel, "custom_off_s") == 0){
  mzrz = getValue(parameter, ',', 1).toInt();
  mzbz = getValue(parameter, ',', 0).toInt();
  mzgz = getValue(parameter, ',', 2).toInt();
  merz = getValue(parameter, ',', 1).toInt();
...
void indexCmd(WebServer &server, WebServer::ConnectionType type, char *, bool)
{
 ...
 server.httpSuccess();
  ...
  if (type != WebServer::HEAD)
  {
     ...
     * amount of RAM. */
    P(mainheader) = "<html><head>"
"<meta http-equiv='Content-Type' content='text/html; charset=windows-1252'>"
"<title>NERD CLOCK ADMIN PAGE</title>"
...
"</style>"
"<script src='http://clock.rbnetworks.de/jstimezonedetect.js' type='text/javascript'></script>"
...
"</head>";
  /* this is a special form of print that outputs from PROGMEM */
    server.printP(mainheader);
   
//ENDE PAGE HEADER
server<<"<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' marginheight='0' marginwidth='0' bgcolor='#FFFFFF'>";
server<<"<center>";
server<<"<table border='0' width='100%' cellspacing='0' cellpadding='0' background='http://clock.rbnetworks.de/img/topbkg.gif'>";
server<<"  <tr>";
server<<"    <td width='50%'><img border='0' src='http://clock.rbnetworks.de/img/topleft.gif' width='205' height='61'></td>";
server<<"    <td width='50%'>";
server<<"      <p align='right'><img border='0' src='http://clock.rbnetworks.de/img/topright.gif' width='205' height='61'></td>";
server<<"  </tr>";
server<<"</table>";
server<<"<table border='0' width='100%' cellspacing='0' cellpadding='0' background='http://clock.rbnetworks.de/img/linebkg.gif'>";
server<<"  <tr>";
server<<"    <td><a href='/'><font color='#FF0000'>STATUS</font></a></td>";
server<<"    <td><font color='#FFFFFF'><a href='/color.html'>COLOR SETTINGS</a></font></td>";
server<<"    <td><font color='#FFFFFF'><a href='/alarm.html'>ALARM SETTINGS</a></font></td>    ";
server<<"    <td><font color='#FFFFFF'><a href='/nightmode.html'>NIGHTMODE SETTINGS</a></font></td>";
server<<"    <td><font color='#FFFFFF'><a href='/network.html'>NETWORK SETTINGS</a></font></td>";
server<<"    <td><font color='#FFFFFF'><a href='/info.html'>INFORMATION</a></font></td>";
server<<"  </tr>";
server<<"</table>";
server<<"<p style='margin-left: 20'><font face='Arial' size='2'>&nbsp;</font></p>";
server<<"<p style='margin-left: 20'>";
server<<"<div><h2> CURRENT TIME: ";if(hour < 10){server<< "0"<< hour;}else{server<<hour;} server<<":";if(minute < 10){server<< "0"<< hour;}else{server<<minute;}server<<":";if(second < 10){server<< "0"<< second;}else{server<<second;} server<<"</h2></div>";
server<<"<p style='margin-left: 20'><font face='Arial' size='2'>&nbsp;</font></p>";
server<<"<table border='0' width='100%' background='http://clock.rbnetworks.de/img/botbkg.gif' cellspacing='0' cellpadding='0'>";
server<<"  <tr>";
server<<"    <td width='100%'><img border='0' src='http://clock.rbnetworks.de/img/botbkg.gif' width='38' height='14'></td>";
server<<"  </tr>";
server<<"</table>";
server<<"</center>";
server<<"</body></html>";
...
server<<"<table border='0' width='100%' cellspacing='0' cellpadding='0' background='http://clock.rbnetworks.de/img/linebkg.gif'>";
server<<"  <tr>";
server<<"    <td><font color='#FFFFFF'><a href='/'>STATUS</a></font></td>";
server<<"    <td><a href='/color.html'><font color='#FF0000'>COLOR SETTINGS</font></a></td>";
server<<"    <td><font color='#FFFFFF'><a href='/alarm.html'>ALARM SETTINGS</a></font></td>    ";
server<<"    <td><font color='#FFFFFF'><a href='/nightmode.html'>NIGHTMODE SETTINGS</a></font></td>";
server<<"    <td><font color='#FFFFFF'><a href='/network.html'>NETWORK SETTINGS</a></font></td>";
server<<"    <td><font color='#FFFFFF'><a href='/info.html'>INFORMATION</a></font></td>";
...
server<<"    <td scope='col'><input type='radio' name='ledmode' id='ledmode' value='0' ";if(ledmode == 0){server <<"checked";}server<<">OFF</td>  ";
server<<"    <td scope='col'><input type='radio' name='ledmode' id='ledmode'  value='1' ";if(ledmode == 1){server <<"checked";}server<<">CUSTOM </td>";
server<<"     <td scope='col'><input type='radio' name='ledmode' id='ledmode'  value='2' ";if(ledmode == 2){server <<"checked";}server<<">RAINBOW</td>";
...
server<<"      <input id='custom_on_h' name='custom_on_h' type='text' value=' ";server<< hzb;server<<",";server<< hzr;server<<",";server<< hzg;server<<"' placeholder='[R,G,B]' /><br>";
server<<"     <input id='custom_on_m' name='custom_on_m' type='text' value=' ";server<< mzb;server<<",";server<< mzr;server<<",";server<< mzg;server<<"' placeholder='[R,G,B]' /><br>";
server<<"      <input id='custom_on_s' name='custom_on_s' type='text' value=' ";server<< szb;server<<",";server<< szr;server<<",";server<< szg;server<<"' placeholder='[R,G,B]' />";
server<<"     </td>  ";
server<<" <td><input type='submit' value='SAVE' /></td> ";
server<<"  </tr>";
server<<"</table>";
...
server<<"    </td>   ";
server<<"    <td scope='col' colspan='3'>";
server<<"      <input id='custom_off_h' name='custom_off_h' type='text' value=' ";server<< hzbz;server<<",";server<< hzrz;server<<",";server<< hzgz;server<<"' placeholder='[R,G,B]' /><br>";
server<<"     <input id='custom_off_m' name='custom_off_m' type='text' value=' ";server<< mzbz;server<<",";server<< mzrz;server<<",";server<< mzgz;server<<"' placeholder='[R,G,B]' /><br>";
server<<"      <input id='custom_off_s' name='custom_off_s' type='text' value=' ";server<< szbz;server<<",";server<< szrz;server<<",";server<< szgz;server<<"' placeholder='[R,G,B]' />";
server<<"     </td>  ";
server<<" <td><input type='submit' value='SAVE' /></td> ";
server<<"  </tr>  ";
