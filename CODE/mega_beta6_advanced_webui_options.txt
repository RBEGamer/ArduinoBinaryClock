...
void setup() {
  ...
  //EEPROM.write(4,10);
EEPROM.write(9,1); //NT
EEPROM.write(10,16); //NT
EEPROM.write(11,17); //NT
...
if(alarm_en == 1){  webui_client.println(" <a href=\'/?toggle_alarm\'\' color='green'><font color='#FF0000'>DISABLE ALARM</font></a>");}else{webui_client.println(" <a href=\'/?toggle_alarm\'\'><font color='#00FF00'>ENABLE ALARM</font></a>");}
 ...
 webui_client.println("<br />");
  ...
  webui_client.print("<br />");
   webui_client.println("<br />");
  webui_client.println("<hr />");webui_client.println("<H2>CLOCK INFO</H2> ");
   
  webui_client.println("CLOCK-IP :"); for (byte thisByte = 0; thisByte < 4; thisByte++) {webui_client.print(ip[thisByte], DEC);if(thisByte < 3){ webui_client.print("."); }}  webui_client.println("<br />");

  webui_client.println("NTPServer-IP :"); for (byte thisByte = 0; thisByte < 4; thisByte++) {webui_client.print(timeServer[thisByte], DEC);if(thisByte < 3){ webui_client.print("."); }} webui_client.println("<br />");

 webui_client.println("CLOCK-MAC :"); for (byte thisByte = 0; thisByte < 6; thisByte++) {webui_client.print(mac[thisByte], HEX);if(thisByte < 5){ webui_client.print(":"); }} webui_client.println("<br />");

  webui_client.println("CLOCK-SOFTWARE VERSION :"); webui_client.print(software_version);  webui_client.println("<br />");
          ...
          webui_client.stop();
           ...
           if(readString.indexOf("?stop_alarm" && alarm_en == 1 && alarm_state == 1) >0){stop_alarm();} 
...
EEPROM.write(13,nthr); //NT
EEPROM.write(14,nthr); //NT
...
EEPROM.write(12,10); //NT
EEPROM.write(13,10); //NT
EEPROM.write(14,10); //NT

EEPROM.write(15,0); //NT
EEPROM.write(16,0); //NT
EEPROM.write(17,0); //NT
...
void stop_alarm(){
    alarm_state = 0;
  while(hour == alarm_h && minute == alarm_m){ //SOLANGE warten bis die zeit vorbei ist
  main_prcess();
  } 
  alarm_process == 0; 
}
